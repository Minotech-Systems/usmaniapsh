...
class Login extends CI_Controller {
    ...
    function validate_login() {
        ...
        $credential_a = array('email' => $email, 'password' => sha1($password), 'level' => 1);
        $query = $this->db->get_where('admin', $credential_a);
        ...
        $credential_a = array('email' => $email, 'password' => sha1($password), 'level' => 0);
        $query = $this->db->get_where('admin', $credential_a);
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session->set_userdata('user_level', $row->level);
            $this->session->set_userdata('branch_id', $row->branch_id);
            $this->session->set_userdata('login_type', 'full_admin');
            redirect(base_url() . 'index.php?full_admin/dashboard', 'refresh');
        }
        
        $credential_a = array('email' => $email, 'password' => sha1($password), 'level' => 2);
        $query = $this->db->get_where('admin', $credential_a);

        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('admin_admission_id', '1');
            $this->session->set_userdata('admin_id', $row->admin_id);
            $this->session->set_userdata('login_user_id', $row->admin_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('user_level', $row->level);
            $this->session->set_userdata('branch_id', $row->branch_id);
            $this->session->set_userdata('login_type', 'newadmission');
            redirect(base_url() . 'index.php?newadmission', 'refresh');
        }
    ...
    //-----------------------------------------------------
    // New Admission Login Functions
    //-----------------------------------------------------
    function new_admission() {
        $this->load->view('backend/new_admission/login');
    }

    function new_admission_login() {
        ...
        $credential_a = array('email' => $email, 'password' => sha1($password));
        $query = $this->db->get_where('users', $credential_a);
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('login_type', 'new_admission');
            redirect(base_url() . 'index.php?new_admission/dashboard', 'refresh');
        }

        $this->session->set_flashdata('flash_message', 'Fail Login');
        $this->session->set_flashdata('login_error', get_phrase('invalid_login'));
        redirect(base_url() . 'index.php?login/new_admission', 'refresh');
    }

    function logout_new_admission_user() {
