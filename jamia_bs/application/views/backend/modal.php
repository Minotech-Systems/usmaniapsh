<script type="text/javascript">
    ...
    function showAjaxModal(url, header)
    {
        ...
        jQuery('#scrollable-modal .modal-body').html('<div style="text-align:center;margin-top:200px;"><img style="width: 100px; opacity: 0.4; " src="http://ekattor-school-erp.com/demo/v7addon/assets/backend/images/straight-loader.gif" /></div>');
...
<div class="modal fade" id="scrollable-modal" tabindex="-1" role="dialog" aria-labelledby="scrollableModalTitle" aria-hidden="true">
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content course-preview-modal">
                <div class="modal-header">
                    ...
                    <button type="button" class="close" data-dismiss="modal" onclick="pageReload()">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body text-center" style="min-height: 300px;">
                    <img style="width: 60px; margin-top: 100px;" src="http://ekattor-school-erp.com/demo/v7addon/assets/backend/images/straight-loader.gif">
                </div>
            </div>
        </div>
    </div>
    <script>
        ...
        function filterCourseFullPage() {
            var url = '<?= base_url('addons/courses') ?>';
        ...
        function course_activity(course_id) {
            $.ajax({
                url: '<?= base_url('addons/courses/index/activity/') ?>' + course_id,
                success: function () {
                    var url = '<?= base_url('addons/courses') ?>';
                    var class_id = $('#class_id').val();
                    var user_id = $('#user_id').val();
                    ...
                    toastr.success('Course Data Updated Successfully');
                    $.ajax({
