<?php $sections = $this->db->get_where('section', array('section_id' => $param1))->result_array(); ?>
<?php foreach ($sections as $sem) { ?>
    <form method="POST" class="d-block ajaxForm" action="<?php echo route('section/update/' . $param1); ?>">
        <div class="form-row">
            ...
            <div class="form-group col-md-12">
                ...
                <select name="semester_id" id="batch" class="form-control "  required>
                    <option value="<?= $sem['semester_id'] ?>">
                        <?php
                        $where = array();
                        $where['id'] = $sem['semester_id'];
                        echo table_column('semester', $where, 'name');
                        ?>
                    </option>
                </select>
            </div>
...
<script>
    ...
    $(".ajaxForm").submit(function (e) {
        var form = $(this);
        ajaxSubmit(e, form, showAllSection);
    });
