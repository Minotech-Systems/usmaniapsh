...
foreach ($fatwa as $edit_data) {
    ...
    $question_data = $this->db->get_where('ifta_question', array('question_id' => $edit_data->question_id))->row();
    ?>
    <div class="row"> 
        <div class="col-md-12">
    ...
    <form action="<?= base_url('fatwa/edit_fatwa/update') ?>" method="post" class="form-horizontal form-groups-bordered validate" id="answer">
        <div class="row">
            ...
            <div class="col-md-7">
                ...
                <input type="text" class="form-control" name="question_title" value="<?= $question_data->title ?>">
                <br>
                <label class="control-label"><?= 'سوال' ?></label>
                <textarea class="form-control ckeditor" required="" name="question"><?= $question_data->question ?></textarea>
                <br>
                <label class="control-label"><?= 'الجواب' ?></label>
                <textarea class="form-control ckeditor" required="" name="fitwa"><?= $edit_data->answer ?></textarea>
                <input type="hidden" name="question_id" value="<?= $edit_data->question_id ?>">
                ...
                <div id="jawab_u_sahi">
                    ...
                    foreach ($edit_persons as $edit_p) {
                        ...
                        <div class="form-group">
                            ...
                            <div><a type="button" href="<?= base_url('fatwa/delete_jawab_sahi/' . $edit_data->answer_id . '/' . $edit_p) ?>" class="btn btn-danger remove">ختم کریں</a></div>
                        </div>
                    <?php } ?>
                </div>
                <center>
