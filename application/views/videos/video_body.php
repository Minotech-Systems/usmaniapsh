<div class="col-lg-9  order-md-1 course_col" id = "video_player_area">
    <div class="text-center">
        <?php
        if (empty($video_id)) {
            $lesson_details = $this->db->get_where('video_lectures')->row_array();
        } else {
            $lesson_details = $this->db->get_where('video_lectures', array('id'=>$video_id))->row_array();
        }

        $lesson_thumbnail_url = 'uploads/course_thumbnail/208486824.jpg'; //$this->lms_model->get_course_by_id($course_id)['thumbnail'];
        ...
        $provider = $lesson_details['video_overview_provider'];
        ...
        if ($lesson_details['video_type'] == 'video' || $lesson_details['video_type'] == '' || $lesson_details['video_type'] == NULL):
        ...
        <?php else: ?>
            <div class="mt-5">
                <a href="<?php echo base_url() . 'uploads/lesson_files/' . $lesson_details['attachment']; ?>" class="btn btn-info text-white" download>
                    <i class="mdi mdi-download font-size-24"></i> <?php echo get_phrase('download') . ' ' . $lesson_details['title']; ?>
                </a>
            </div>
        <?php endif; ?>
    </div>
