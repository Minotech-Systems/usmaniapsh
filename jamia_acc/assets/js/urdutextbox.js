// UrduTextBox.js is javascript used to enable HTML textbox and textarea to enable editing of Urdu text in it.
...
// If you find this component useful and able to earn money by using it, it is required to submit 7% of revenue to developer of this component. For details email to naseem.amjad[at]gmail.com

//Known Limitation: TextArea in Internet Explorer does not move cursor to next line on pressing Enter Key if the line contains non visible characters like space or no charachter at all.
...
function com_ajsoftpk_KeyPress(textbox,evt){
	...
	if(com_ajsoftpk_isValidAlphabet(keyChar))
		{
			var hebChar=com_ajsoftpk_getNextStateUrduPhoneticLayout(com_ajsoftpk_findLastChar(textbox),keyChar);
			if(hebChar==keyChar)
			{
				com_ajsoftpk_replaceEndOfWord(textbox,com_ajsoftpk_findLastChar(textbox));
			}
			
			com_ajsoftpk_insertAtCaret(textbox,hebChar);
