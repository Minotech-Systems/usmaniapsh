<?php
$edit_data = $this->db->get_where('jamia_expenses', array('id' => $param2))->result_array();
foreach ($edit_data as $data) {
    ...
    <?php echo $this->crud_model->get_column_name_by_id('sub_child_expense_category', 'id', $data['sub_child_expense_id']); ?>
                    </div>
                </div>
                <div class="panel-body">
    <?php echo form_open(base_url() . 'index.php?admin/jamia_expenses/update/'.$param2, array('class' => 'form-horizontal form-groups-bordered validate', 'enctype' => 'multipart/form-data')); ?>
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <select name="sub_child_expense_id" class="form-control">
                                ...
                                <?php
                                $kata_banam = $this->db->get('sub_child_expense_category')->result_array();
                                foreach ($kata_banam as $row):
                                    ?>
                                    <option value="<?php echo $row['id']; ?>" <?php if ($row['id'] == $data['sub_child_expense_id']) echo 'selected'; ?>>
                                    <?php echo $row['name']; ?>
                                    </option>
    <?php endforeach; ?>
                            </select> 
